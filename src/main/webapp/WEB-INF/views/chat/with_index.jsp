...
<%
	...
	Integer select  = 1; // 물물교환과, 공동구매를 구별하는 정수이다. 0: 물물교환, 1: 공동구매
	//List<ChatRoomVO> chatList = (List<ChatRoomVO>)request.getAttribute("chatList");
...
<jsp:include page="with_chat.jsp" />
<jsp:include page="with_chat_write.jsp" />
	...
	var csrfTokenValue="${_csrf.token}";
		...
		function movePage(page){
			...
			if('${localSearch}'||'${titleSearch}') {
				location.href="change_index?page="+page+"&localSearch="+encodeURIComponent(localSearch)+"&titleSearch="+encodeURIComponent(titleSearch)+"&select=1";
			}
			
			location.href="change_index?page="+page+"&select=1";
		}
		...
		var connect = function(){
			...
			stompClient.connect({}, function() { // 접속
				...
				subscription = stompClient.subscribe("/sub/chat/room/"+roomNum+"/select/with", function(message) {
		...
		closeSpan.onclick = function() {
			if(isError) {
				stompClient.send("/pub/chat/message/with", {}, JSON.stringify({type:'EXIT', roomId: roomNum, sender:"${nickname}", userNum : "${userNum}",message:"byError"}));
			} else {
				stompClient.send("/pub/chat/message/with", {}, JSON.stringify({type:'EXIT', roomId: roomNum, sender:"${nickname}", userNum : "${userNum}"}));
			}
		...
		submitBtn.onclick = function(){
				...
				if(input !=""){
					...
					stompClient.send("/pub/chat/message/with", {}, JSON.stringify({type:'TALK', roomId : roomNum, sender:"${nickname}", message: input, userNum : "${userNum}"}));
					input = "";
				}
				// 채팅창으로 키보드커서 이동
				$('.typingbox').focus();
